...
import { UsersTableHeaderData, UsersTableBodyData } from '@/helper/data';
        ...
        <DashboardLayout>
            <div>
                <div className="flex flex-col gap-5 md:flex-row md:items-center justify-between mb-8">
                    <div>
                        <h5 className="font-semibold text-xl md:text-2xl leading-tight text-[#0F1729]">Users Management</h5>
                        <h6 className="font-normal text-base md:text-lg leading-tight text-[#161819]">Manage individual sellers, dealers and admins</h6>
                    </div>
                    <Button pre={true} type="button" text="Invite Admin"
                        ...
                        <table className="w-full border-collapse">
                            <thead className="sticky -top-[2px] z-10 bg-[#F1F5F9]">
                                <tr className="border border-[#E1E7EF] rounded-t-[10px]">
                                    {UsersTableHeaderData.map((tabular) => {
                                        return (
                            ...
                            <tbody>
                                {UsersTableBodyData.map((dataRow, index) => (
                                    <tr key={index} className="border border-[#E1E7EF]">
                                        <td className="px-4 py-3 text-[#0F1729] text-sm lg:text-base font-semibold">{dataRow.username}</td>
                                        <td className="px-4 py-3 text-[#0F1729] text-sm lg:text-base">{dataRow.email}</td>
                                        <td className={`px-4 py-3 text-sm lg:text-base`}>
                                            <span className={`rounded-full h-[32px] px-[15px] py-[10px] border border-[#E7F0FE]
                                                ${dataRow.type == "Individual" ? "text-[#0F1729]" : "text-[#2474F5]"}`}>
                                                {dataRow.type}
                                            </span>
                                        </td>
